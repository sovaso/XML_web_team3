...
public class JwtUtil {
	...
	private final long EXPIRES_IN = 12 * 60 * 60 * 1000 * 2;
	...
	public String generateToken(UserDetails userDetails) {
		...
		return Jwts.builder().setClaims(claims).setSubject(userDetails.getUsername()).setIssuedAt(new Date())
				...
				.signWith(SIGNATURE_ALGORITHM, SECRET_KEY_BASE64).compact();
	}
	...
	private Claims getAllClaimsFromToken(String token) {
		Claims claims;
		try {
			claims = Jwts.parser().setSigningKey(SECRET_KEY_BASE64).parseClaimsJws(token).getBody();
		} catch (Exception e) {
			claims = null;
		}
		return claims;
	...
	public Boolean validateToken(String token, UserDetails userDetails) {
		...
		return (username != null && username.equals(userDetails.getUsername()) && !expired);
	}
